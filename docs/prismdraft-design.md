# PrismDraft Design Document

Source: https://docs.google.com/document/d/1gocSWlCi_SNYCkqoY9bJzcjeV5UcEzL5ovkk7YyUKnA/edit?usp=sharing
Synced at: 2026-05-30T20:33:12.1446189+08:00
Google Doc ID: 1gocSWlCi_SNYCkqoY9bJzcjeV5UcEzL5ovkk7YyUKnA
...
在片元着色器中计算面法线与世界光源方向的点积（  ）。不使用连续渐变，直接使用硬阶跃函数（Step/Threshold），模拟矢量插画中的硬投影：  
最终像素颜色 =   。
...
阶段 1：内存骨架与基础视口 (基础奠定)
      ...
      * AI 任务清单：
      1. 编写 AppContext 及内存分配、安全扩容与释放函数。
      2. 使用 Raylib 初始化基础窗口、视口、等距视角（Isometric）风格倾向的轨道相机控制。
      3. 实现一个手动构造基础标准立方体（Cube）半边结构的测试用例，并将其正确打平渲染。
阶段 2：渲染管线打通 (视觉闭环)
      * 目标： 实现完全不需要任何外部贴图的矢量扁平色块渲染与硬几何投影。
      * AI 任务清单：
      1. 编写自定义加载 GLSL 顶点的 Forward Shader，实现平直顶点色与无渐变阶跃（Hard-Step）矢量光影。
      2. 配置离屏 FBO 渲染管线，将场景的深度和世界空间法线写入独立的渲染目标（Render Texture）。
      3. 编写基于深度与法线的边界分割后处理着色器，完成整体图形化海报画风在软件内部的呈现。
阶段 3：拓扑编辑工具集开发 (核心功能)
      * 目标： 让工具具备真正的手工几何建模能力。
      * AI 任务清单：
      1. 编写基于屏幕射线的点、边、面拾取（Raycasting）和高亮选中逻辑。
      2. 基于扁平数组半边数据结构，严格实现 Extrude（面挤出）算法，要求正确更新和缝合所有受影响的 HalfEdge, Vertex, Face 索引，杜绝野索引。
      3. 实现 Bevel（硬倒角）与 Inset（内插面）的核心几何数学计算。
阶段 4：序列化与工程闭环 (交付阶段)
      * 目标： 资产能够安全走出本工具，被其他引擎高效读取，达成完全独立的生态链。
      * AI 任务清单：
      1. 编写一个简单的基于 PRS 关键帧的时间轴控制器，允许对模型进行基础的动态演示。
      2. 编写通用纯 C 的 glTF 2.0 / GLB 导出器，将打平后的顶点流及 COLOR_0 写入二进制文件，确保在第三方查看器或游戏引擎中自带完整的顶点色。
